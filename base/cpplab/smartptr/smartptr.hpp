...
public:
  explicit SmartPtr(T *pPtr = nullptr) : mPtr(pPtr) {
    if (pPtr) {
  ...
  // move constructor
  template <typename U> SmartPtr(const SmartPtr<U> &&pVal) {
    mPtr = pVal.mPtr;
    if (mPtr) {
  ...
  template <typename U> SmartPtr(const SmartPtr<U> &pVal, T *ptr) {
    mPtr = ptr;
    if (mPtr) {
      pVal.mSharedCount->increaseCount();
      mSharedCount = pVal.mSharedCount;
  ...
  void test() { std::cout << "smartptrtest"; };
...
template <typename T> void swap(SmartPtr<T> lhs, SmartPtr<T> rhs) noexcept {
  lhs.swap(rhs);
}
